...
def sql_initialize(dbName):
        if not os.path.isfile(dbName):
                ...
                try:
                        ...
                        cur = con.cursor()
                        cur.execute("CREATE TABLE clients(clientMachineId TEXT , machineName TEXT, applicationId TEXT, skuId TEXT, licenseStatus TEXT, lastRequestTime INTEGER, kmsEpid TEXT, requestCount INTEGER, lastHost TEXT, PRIMARY KEY(clientMachineId, skuId))")
...
skuId, licenseStatus, lastRequestTime, requestCount, lastHost) VALUES (:clientMachineId, :machineName, :appId, :skuId, :licenseStatus, :requestTime, 1, :host);", infoDict)
                        else:
                                # Update data.
...
def sql_update_epid(dbName, kmsRequest, response, skuId):
        cmid = str(kmsRequest['clientMachineId'].get())
        con = None
        try:
                con = sqlite3.connect(dbName)
                cur = con.cursor()
                cur.execute("SELECT * FROM clients WHERE clientMachineId=? AND skuId=?;", (cmid, skuId))
                try:
                        data = cur.fetchone()
                        cur.execute("UPDATE clients SET kmsEpid=? WHERE \
clientMachineId=? AND skuId=?;", (str(response["kmsEpid"].decode('utf-16le')), cmid, skuId))
